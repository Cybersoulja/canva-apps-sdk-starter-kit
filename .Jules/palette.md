...
## 2024-05-24 - Async Design Modifications
**Learning:** Canva design SDK functions that modify the design (like `addElement`) are asynchronous and return Promises.
**Action:** Always wrap these operations in a `try/finally` block and bind them to the `loading` state of UI components (like `Button`) to provide immediate visual feedback.
