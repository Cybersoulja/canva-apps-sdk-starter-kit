...
## 2024-05-25 - [Button Loading State for SDK calls]
**Learning:** Functions that interact with the Canva platform or design (like `addElement` or `requestOpenExternalUrl`) are asynchronous. If a user interacts with a button that triggers these functions, they expect immediate feedback.
**Action:** Always wrap these SDK calls in a `try/finally` block to reliably manage a loading state (`isAddingElement`) on the triggering Button component to improve perceived performance and prevent duplicate submissions.
